...
\hfill
\includegraphics[width=0.4\textwidth]{img/Logo_CentraleSupélec.svg.png}


% Title
\HRule \\[0.4cm]
{ \LARGE 
  \textbf{Deep Learning: Article study}\\[0.4cm]
  \emph{Explaining and Harnessing Adversarial Examples}\\[0.4cm]
}
\HRule \\[1.5cm]
...
\begin{figure}[h]
    ...
    \caption{A demonstration of fast adversarial example generation. By adding an imperceptibly small vector whose elements are equal to
the sign of the elements of the gradient of the cost function with respect to the input, we can change the detector's classification of the image \cite{main}.}
...
\end{equation}
To maximize the deviation in activation subject to the constraint \ref{eq:constraint}, the authors state that one assigns $\bm{\eta} = \text{sign}(\bm{w})$. If the weight vector $\bm{w}$ has $n$ dimensions and the average magnitude of an element is $m$, the activation will then grow by $\varepsilon mn$. This demonstrates that in high-dimensional spaces (large $n$), many small changes can accumulate to produce a massive change in the output, sufficient to cause errors in the outputs of the model.
...
Adversarial training is a regularization technique that involves training the model on a mixture of clean and adversarial examples, effectively minimizing the worst-case error when the data is perturbed by an adversary sample. 
...
\begin{equation}
    \mathbb{E}_{x,y \sim p_{data}} \zeta(y(\varepsilon ||w||_1 - w^\top x - b))
\end{equation}
This formulation resembles $L^1$ regularization but adds the penalty inside the activation function rather than subtracting it from the cost, meaning the penalty effectively disappears if the model's prediction is confident enough.
...
\begin{equation}
    \tilde{J}(\theta, x, y) = \alpha J(\theta, x, y) + (1-\alpha) J(\theta, x + \varepsilon \cdot \text{sign}(\nabla_x J(\theta, x, y)), y)
\end{equation}
...
In the article, the authors applied FGSM to several standard models to demonstrate the universality of the vulnerability. On the ImageNet dataset, the authors showed that applying the method to "GoogLeNet" could cause the model to misclassify a panda as a gibbon with 99.3\% confidence, despite the perturbation being indistinguishable to the human eye. Such an example is shown in figure \ref{fig:screen_capture}.
...
%\textbf{Key concepts :} OD more complex than Classif, Gradient of detector not really the same (need details see code), need explanation on detector/dataset/setup/metrics, several types of outputs instead of just misclass, adv train more simple here compared to theoretical formulation, highlight balance between clean perf and adv, transferability with SSD, is Linf constraint that telling ?

Object detection is obviously a more complicated task than classification.  In addition to just assigning a class to images, object detectors often have to handle multiple objects in a single image, determining both their respective positions in bounding boxes and their respective classes. To apply FGSM to our images, we were able to use PyTorch's \textit{requires\_grad} function to compute the gradient of our detector on the target images.
...
\begin{figure}[!htb]
    \centering
    \subfigure[Clean Image]{\includegraphics[width=0.24\textwidth]{img/009909_0.00.jpg}} 
    \subfigure[$\varepsilon = 0.02$]{\includegraphics[width=0.24\textwidth]{img/009909_0.02.jpg}} 
    %\subfigure[$\varepsilon = 0.0$]{\includegraphics[width=0.24\textwidth]{img/009909_0.04.jpg}}
    \subfigure[$\varepsilon = 0.06$]{\includegraphics[width=0.24\textwidth]{img/009909_0.06.jpg}}
    \subfigure[$\varepsilon = 0.08$]{\includegraphics[width=0.24\textwidth]{img/009909_0.08.jpg}}
    \subfigure[$\varepsilon = 0.10$]{\includegraphics[width=0.24\textwidth]{img/009909_0.10.jpg}}
    \subfigure[$\varepsilon = 0.13$]{\includegraphics[width=0.24\textwidth]{img/009909_0.13.jpg}}
    \subfigure[$\varepsilon = 0.17$]{\includegraphics[width=0.24\textwidth]{img/009909_0.17.jpg}}
    \subfigure[$\varepsilon = 0.20$]{\includegraphics[width=0.24\textwidth]{img/009909_0.20.jpg}}
    \caption{Example of detections provided by the YOLO11n Model of an image with increasing perturbation value.}
    \label{fig:exdet}
\end{figure}
...
\begin{figure}[!htb]
    \centering
    \subfigure[Clean Image]{\includegraphics[width=0.25\textwidth]{img/output/000018.jpg}} 
    \subfigure[Clean Image]{\includegraphics[width=0.25\textwidth]{img/output/000049.jpg}} 
    \subfigure[Clean Image]{\includegraphics[width=0.25\textwidth]{img/009909_0.00.jpg}}
    \subfigure[Adversarial Image (Fabrication Output)]{\includegraphics[width=0.25\textwidth]{img/output/det_000018.jpg}} 
    \subfigure[Adversarial Image (Mislabeling Output)]{\includegraphics[width=0.25\textwidth]{img/output/det_000049.jpg}} 
    \subfigure[Adversarial Image (Vanishing Output)]{\includegraphics[width=0.25\textwidth]{img/009909_0.13.jpg}}
...
\begin{figure}[!htb]
    \centering
    \subfigure[]{\includegraphics[width=0.49\textwidth]{img/transferability.png}} 
    \subfigure[]{\includegraphics[width=0.49\textwidth]{img/mapratio.png}} 
...
\begin{figure}[!htb]
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=0.99\textwidth]{img/advtrained.png}
     \caption{Comparison of YOLOv11n Model Performance after Adversarial Training.}\label{Fig:advtrain}
   \end{minipage}\hfill
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=0.99\textwidth]{img/cleanperf.png}
     \caption{Comparison of YOLOv11n Model Performance on the clean images only after Adversarial Training.}\label{Fig:cleanperf}
   \end{minipage}
\end{figure}

Figure \ref{Fig:advtrain} illustrates the performance of YOLO11n variants following adversarial training across various perturbation magnitudes. A fundamental trade-off is observed: as the detectors' robustness to attacks increases, their efficacy on clean images declines. Preserving high performance on unperturbed data is critical, as these images constitute the predominant input in real-world scenarios. Consequently, balancing standard accuracy with adversarial robustness is essential for effective adversarial training. To quantify this impact, Figure \ref{Fig:cleanperf} details the performance of each detector specifically on clean images.
